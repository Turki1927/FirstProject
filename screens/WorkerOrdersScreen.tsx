...
const WorkerOrdersScreen = () => {
  ...
  const renderStatus = (status: string) => {
    switch (status) {
      case 'pending':
        return <Text style={[styles.status, { color: '#f39c12' }]}>قيد الانتظار ⏳</Text>;
      case 'accepted':
        return <Text style={[styles.status, { color: '#2ecc71' }]}>تم القبول ✅</Text>;
      case 'rejected':
        return <Text style={[styles.status, { color: '#e74c3c' }]}>مرفوض ❌</Text>;
      case 'cancelled':
        return <Text style={[styles.status, { color: '#999' }]}>🚫 تم الإلغاء</Text>;
      default:
